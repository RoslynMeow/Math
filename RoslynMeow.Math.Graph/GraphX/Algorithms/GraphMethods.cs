...
using RoslynMeow.Math.Graph.Core;
using RoslynMeow.Math.Graph.Util;

namespace RoslynMeow.Math.Graph.Algorithms
{
    /// <summary>
    ...
    public static class GraphMethods
    {
            ...
            where TWeight : IComparable<TWeight>
        {
            ...
            void StrongConnect(NodeType v)
            {
                ...
                foreach (var w in adj[v])
                {
                    ...
                    {
                        StrongConnect(w);
                        lowlink[v] = global::System.Math.Min(lowlink[v], lowlink[w]);
                    }
                    else if (onstack.Contains(w))
                    {
                        lowlink[v] = global::System.Math.Min(lowlink[v], indices[w]);
                    }
                }
            ...
            void Dfs(NodeType u)
            {
                ...
                foreach (var v in adj[u])
                {
                    ...
                    if (!visited[v])
                    {
                        parent[v] = u;
                        Dfs(v);
                        low[u] = global::System.Math.Min(low[u], low[v]);
                    ...
                    {
                        // 返祖边 / back edge
                        low[u] = global::System.Math.Min(low[u], disc[v]);
                    }
                }
            }
            ...
            where NodeType : IEquatable<NodeType>
        {
            ...
            foreach (var (U, V, W) in graph.WeightedEdges)
            {
                ...
                var key = (U, V);
                if (residual.ContainsKey(key)) residual[key] = global::System.Math.Max(residual[key], W);
                else residual[key] = W;
                var rev = (V, U);
            ...
            while (true)
            {
                ...
                {
                    var ekey = edgeFrom[cur]; var cap = residual[ekey]; bottleneck = global::System.Math.Min(bottleneck, cap); cur = parent[cur];
                }
            ...
            while (true)
            {
                ...
                long Dfs(NodeType u, long f)
                {
                    ...
                    for (int i = iters[u]; i < adj[u].Count; i++)
                    {
                        ...
                        if (capuv <= 0) continue;
                        var pushed = Dfs(v, global::System.Math.Min(f, capuv));
                        if (pushed > 0)
                        {
